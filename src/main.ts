...
function createWindow() {
  ...
  const win = new BrowserWindow({
    ...
    // macOS keeps its native frame (traffic lights); Windows/Linux get a
    // frameless window with an in-app title bar drawn by the renderer.
    frame: isMac,
    webPreferences: {
  ...
  if (process.platform === "darwin") {
    // macOS requires an application menu for standard app behavior (Cmd+Q, etc).
